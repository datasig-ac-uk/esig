...
{
    using container_type = typename esig::algebra::py_lie_key::container_type;
    esig::dimn_t size;
    esig::dimn_t current;
    esig::deg_t width;
    ...
    {
        using esig::algebra::py_lie_letter;
        if (py::len(obj) != 2) {
...
esig::algebra::py_lie_key make_lie_key(const py::args& args, const py::kwargs& kwargs)
{
    esig::deg_t width = 0;
    ...
    if (py::isinstance<py::int_>(args[0])) {
        ...
        return esig::algebra::py_lie_key(width, letter);
    }
    ...
    return esig::algebra::py_lie_key(helper.get_width(), helper(args[0]));
}
