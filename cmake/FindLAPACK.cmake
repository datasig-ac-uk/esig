...
function(_detect_mkl_by_package_config)
  if(NOT TARGET MKL::MKL)
    # Try to detect oneMKL and later versions by their cmake package configuration.
    # Set the possible options beforehand:
    # MKL_LINK Values:  static, dynamic, sdl
    if(BLA_STATIC)
      set(MKL_LINK "static")
    elseif("${BLA_VENDOR}" MATCHES "_dyn")
      # This is correct, cmake uses suffix 'dyn' for 'single dynamic library',
      # short 'sdl'. Not to be confused with 'dynamic' linkage.
      set(MKL_LINK "sdl")
    else()
      set(MKL_LINK "dynamic")
    endif()
    # ENABLE_BLAS95:      Enables BLAS Fortran95 API
    set(ENABLE_BLAS95 "${BLA_F95}")
    # ENABLE_LAPACK95:    Enables LAPACK Fortran95 API
    set(ENABLE_LAPACK95 "${BLA_F95}")
    # MKL_ARCH Values:  ia32 intel64
    if("${BLA_SIZEOF_INTEGER}" STREQUAL "4" OR "${BLA_VENDOR}" MATCHES "32")
      set(MKL_ARCH "ia32")
    elseif("${BLA_SIZEOF_INTEGER}" STREQUAL "8" OR "${BLA_VENDOR}" MATCHES "64")
      set(MKL_ARCH "intel64")
    endif()
    # MKL_INTERFACE Values:  lp64, ilp64
    if("${BLA_VENDOR}" MATCHES "64lp")
      set(MKL_INTERFACE "lp64")
    elseif("${BLA_VENDOR}" MATCHES "64ilp")
      set(MKL_INTERFACE "ilp64")
    endif()
    # MKL_THREADING Values:  sequential, intel_thread (Intel OpenMP), gnu_thread (GNU OpenMP), pgi_thread (PGI OpenMP), tbb_thread
    if("${BLA_VENDOR}" MATCHES "_seq")
      set(MKL_THREADING "sequential")
    else()
      # TODO: Add support for the different threading options instead of just falling back to 'intel_thread':
      set(MKL_THREADING "intel_thread")
    endif()

    # Other possible MKL configuration options:
    # MKL_MPI Values:  intelmpi, mpich, openmpi, msmpi, mshpc
  ...
  if(TARGET MKL::MKL)
    set(LAPACK_FOUND ${MKL_FOUND} PARENT_SCOPE)
    set(LAPACK_LIBRARIES MKL::MKL PARENT_SCOPE)
  endif()
endfunction()
...
if(NOT LAPACK_NOT_FOUND_MESSAGE)
  ...
  if(NOT LAPACK_LIBRARIES
    ...
    if(DEFINED ENV{MKL_ROOT})
      file(TO_CMAKE_PATH "$ENV{MKL_ROOT}" LAPACK_mkl_MKLROOT)
    endif()
